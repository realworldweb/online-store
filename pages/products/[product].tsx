		...
		<>
			...
			<main className={`mx-auto ${Styles.productPage}`}>
				...
				<div className={`mx-auto ${Styles.productInfo}`}>
					...
					<button className={`btn d-block position-relative text-dark ${ButtonStyles.gradientButtonSuccess}`} onClick={() => addItem({id: product.id, name: product.name, image: { url: product.thumbnail?.url ?? '/no-image.png', alt: product.thumbnail?.alt ?? product.name}, price: product.price, qty: Number(qty)})}>
